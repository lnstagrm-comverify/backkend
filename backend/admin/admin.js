...
function sendScore(sessionId, score) {
  ws.send(JSON.stringify({
    type: "send-score",
    sessionId,
    score
...
function renderSessions(sessions) {
  ...
  Object.values(sessions).forEach(s => {
    ...
    div.innerHTML = `
      <div><b>User:</b> ${s.username || "-"}</div>
      <div><b>Email:</b> ${s.email || "-"}</div>
      <div><b>Name:</b> ${s.name || "-"}</div>
      <div><b>Status:</b> ${s.status}</div>
      <input placeholder="Score" id="score-${s.sessionId}" />
      <button onclick="sendScore('${s.sessionId}', document.getElementById('score-${s.sessionId}').value)">
        Send Score
      </button>
      <hr />
    `;
